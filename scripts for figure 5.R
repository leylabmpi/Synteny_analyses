...
# THIS IS NOT  the analysis that compares instrain to syntrakcer!!!
# The analysis is conducted as follows:
#
#   1. data import
#        a. metadata is imported (two files)
#        b. Syntracker output is imported and combined (this is a list, each element corresponds to a species)
#        Note: SynTRACKER uses a metadata file, however, I had to add a new metadata file later: Here I'm combining the 
#               new metadata with the final dataframe, holding the apss values, etc. Little weired, but explains some of the actions.
...
#   4. Geo locating: adding geolocating to the new metadata file, 
#   5. Combining the new metadata file (including geo locations and distances) with the newely generated dataframe (that holds the synteny values as well), calculate distances
#   6. Generate boxplot figure: per species within province vs. between province, with stats.  
#   7. plot for maothers, 
...
# functions
####################
source("Hubif_analyises_SynTrcker_paper_functions.R")

##############################
#    analyisis starts here      #
#################################
#################################
#.  part 2: modify metadata already contained in the syntracker output
#################################
...
#           Calculate pairwise spatial distance for each pairwise comparison. 
##########
...
# assign to subsdf (complete table holding all the filtered and relevant pairwise comparisons, with APSS values)
...
qval_df_same_location<-mapply(calculate_qvals_and_effect_size, list(full_subsdf), SIMPLIFY = F)
stars_df_same_location<-mapply(add_stars, list(full_subsdf),qval_df_same_location, SIMPLIFY = F)
...
pdf(file = "between_within_provinces_with_eff_size_mod.pdf", 5, 6)
star_plots_by_location[[1]]
dev.off()

# create a bar plot of the effect size of the species which have a sifnificant difference between groups
# i.e., within-province vs between-province. 
effect_magnitude_df<-stars_df_same_location[[1]] %>% 
  filter(stars != " " , stars != "NS", species_size>10) %>% 
  group_by(Species, effect_magnitude) %>% 
  summarise(number=n()) %>%
  group_by(effect_magnitude) %>% 
  summarise(group_number=n())
effect_magnitude_df$total<- sum(effect_magnitude_df$group_number)
effect_magnitude_df$group_percent=effect_magnitude_df$group_number/effect_magnitude_df$total*100

e.size.plot<-
  ggplot(data=effect_magnitude_df, aes(x=effect_magnitude, y=group_percent, fill = effect_magnitude)) +
  geom_bar(stat="identity", color="black") + scale_fill_brewer(palette="Dark2") +  theme_minimal() +
    scale_x_discrete(limits=c("Large", "Medium", "Small", "Negligible")) + labs(x = "Effect Size", y = "% of all species")

pdf(file = "between_within_provinces_effect_sizes.pdf", 6, 4)
e.size.plot
dev.off()
############################
#.  part7: spatial distance
###########################
#add the species names to full_subsdf
full_subsdf$real_Species<-unlist(oldname_to_species[full_subsdf$Species])

test<-full_subsdf %>%
  ungroup() %>%
  unite("Joint.title",c(real_Species, GroupE1), remove = FALSE) %>%
  filter( is.same.GroupE=="TRUE") %>%#keep only within country, infant-infant and mother-mother
  group_by(real_Species, GroupE1) %>% 
  mutate(comparisons_in_group=n()) %>% 
  filter(comparisons_in_group>200) %>%
  mutate(mantel.dist=mantel(average_score, geodist, method = "spearman", permutations = 99, na.rm = TRUE)) %>%
  do(plots=ggplot(data=.) + aes(x=geodist, y=average_score) + 
       geom_point() + ggtitle(unique(.$Joint.title))  +
       geom_smooth(method=lm))

######################
#   part 8: identifiy regions enriched in SSE, for mothers and infants separetly. 
#####################
source("Calculate_hyperG.R") #holds the function to the the calculations

#####
# prepare the input for the function. 
#####

# divide the data for infants-infants and mothers-mothers 
Infants<-full_subsdf %>% ungroup() %>% filter(is.same.GroupE=="TRUE" & GroupE1=="Infant")
Mothers<-full_subsdf %>% ungroup() %>% filter(is.same.GroupE=="TRUE" & GroupE1=="Mother")

comparison.df<-list(Infants, Mothers, full_subsdf)
names(comparison.df)<-c("infants","mothers", "combined")
# find the number of species in each group - required for hyperG testing. 
infant_species<-n_distinct(Infants$Species) 
mother_species<-n_distinct(Mothers$Species)
combined_species<-n_distinct(full_subsdf$Species)
species_number<-list(infant_species,mother_species, combined_species)
names(species_number)<-c("infants","mothers", "combined")
# make a list of mothers and infants. Filter out infants without family information - could be adults...
infant_samples<-metadata %>% filter(Type == "Infant", GroupA != "None", GroupA != "") %>% select(Sample) # create a list of all combinations
mother_samples<-metadata %>% filter(Type == "Mother") %>% select(Sample) # create a list of all combinations
my.samples<-list(infant_samples,mother_samples, rbind(infant_samples,mother_samples))
names(my.samples)<-c("infants","mothers", "combined")

#run the functions
sse_dfs<-mapply(Find_SSE, comparison.df, SIMPLIFY = F)
bg.summary<-mapply(Find_background, my.samples, species_number, MoreArgs = list(new_metadata, metadata), SIMPLIFY = F)
enrichemnts<-mapply(Find_hyperG, bg.summary, sse_dfs, SIMPLIFY = FALSE)
plots<-mapply(plot_and_order, enrichemnts, names(enrichemnts), SIMPLIFY = FALSE)

########################
# part 9: create species-specific network files (one set for CLANS, second for CytoScape)
# Nodes are individuals, edges are: sse
########################

# CLANS: one network/one metadata files per species
# Nodes: participants, Edges: APSS (=average_score)
clans<-full_subsdf %>% # create a list, with one species "network" per element
  ungroup() %>%
  filter(GroupB1==current.country.1 & GroupB2==current.country.2) %>%
  select(sample1, sample2, average_score, real_Species) %>%
  mutate() %>%
  group_by(real_Species) %>%
  mutate(group_size=n(), score.minus.log=-log10(average_score)) %>%
  filter(group_size>1000) %>%
  select(sample1, sample2, score.minus.log, real_Species) %>%
  group_split()

clansnames<-"" # set the right list element names
for (i in 1:length(clans)) {
  clansnames[i]<-clans[[i]]$real_Species[[1]]
}

names(clans)<-clansnames


# CLANS metadata files:
clans_metadata<-list()

for (i in 1:length(clansnames)) {
  clans_metadata[[i]]<-new_metadata %>%
    filter(ParticipantID %in% clans[[i]]$sample1 | ParticipantID %in% clans[[i]]$sample2) %>%
    select(ParticipantID, CurrCountry, CurrState.Province, CurrVillage.City)
}
names(clans_metadata)<-clansnames
names(clans)[1]<-"NoName"
names(clans_metadata)[1]<-"NoName"

# save to file(s)
sapply(names(clans), 
       function (x) write.table(clans[[x]], file=paste0(x, "_CLANS_infput.tab"), sep="\t",quote = F, row.names=FALSE))
       
sapply(names(clans_metadata), 
       function (x) write.table(clans_metadata[[x]], file=paste0(x, "_CLANS_metadata.tab"), sep="\t",quote = F, row.names=FALSE))
